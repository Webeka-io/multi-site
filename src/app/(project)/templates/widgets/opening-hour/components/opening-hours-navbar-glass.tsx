  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          className="relative h-10 px-4 bg-white/10 backdrop-blur-md border border-amber-600 hover:bg-white/20 transition-all duration-300 rounded-full shadow-lg"
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-amber-600 sm:inline">{currentlyOpen ? "Ouvert" : "Fermé"}</span>
            <ChevronDown className={` text-amber-600 h-3 w-3 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} />
          </div>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent
