...
function parseRemove(params: URLSearchParams) {
  const out: string[] = [];
  // supporte &remove=sel1,sel2&remove=sel3…
  ...
  // Supprime le badge Framer par défaut
  out.push("#__framer-badge-container", ".__framer-badge-container");
  // dédoublonne et limite (sécurité)
...
export async function GET(req: NextRequest) {
  ...
  // 2) Option : couper le JS de la page proxifiée
  if (disableJs) {
    html = html.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi, "");
    html = html.replace(/<link[^>]+rel=["']modulepreload["'][^>]*>/gi, "");
  }

  // 3) Injection traduction + suppression (runtime + Shadow DOM)
  if (translateTo || removeList.length) {
    const injected =
      ...
      'if(!TARGET_LANG){running=false;return;}' + // si pas de traduction demandée, on s’arrête après suppression
    ...
    html = /<\/body>/i.test(html) ? html.replace(/<\/body>/i, injected + "</body>") : html + injected;
    // facultatif: lang="fr" si on traduit
    if (translateTo) {
