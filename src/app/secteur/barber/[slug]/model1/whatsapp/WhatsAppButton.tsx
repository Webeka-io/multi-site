"use client";

import { useState, useEffect, CSSProperties } from "react";
import { createPortal } from "react-dom";

interface WhatsAppButtonProps {
  phoneE164: string;
  message?: string;
  label?: string;
  className?: string;
  customPosition?: CSSProperties; // { bottom: 24, right: 10 } etc.
  theme?: "dark" | "light";
}

export default function WhatsAppButton({
  phoneE164,
  message = "Bonjour, je veux mon site !",
  label = "WhatsApp",
  className,
  customPosition = { bottom: 24, right: 10 },
  theme = "dark",
}: WhatsAppButtonProps) {
  const [mounted, setMounted] = useState(false);
  const [isPopupOpen, setIsPopupOpen] = useState(false);

  useEffect(() => setMounted(true), []);
  if (!phoneE164) return null;

  const cleanPhone = phoneE164.replace(/[\s\-()]/g, "");
  const whatsappUrl = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(message)}`;

  const baseBtnStyle: CSSProperties =
    theme === "dark"
      ? { backgroundColor: "#000", color: "#ed0000", border: "1px solid #ed0000" }
      : { backgroundColor: "#000", color: "#ed0000", border: "1px solid #ed0000" };

  const handleOpenWhatsApp = () => {
    window.open(whatsappUrl, "_blank", "noopener,noreferrer");
    setIsPopupOpen(false);
  };

  const content = (
    <>
      {/* Bouton flottant */}
      <button
        onClick={() => setIsPopupOpen(true)}
        aria-label={`${label} - Ouvrir une conversation`}
        className={`fixed flex items-center justify-center rounded-full shadow transition-transform duration-200 ${className ?? ""}`}
        style={{
          width: 43,
          height: 43,
          zIndex: 1050,           // ou className: z-[1050]
          cursor: "pointer",
          ...baseBtnStyle,
          ...customPosition,      // top/bottom/left/right en px
        }}
        onMouseEnter={(e) => {
          e.currentTarget.style.transform = "scale(1.08)";
          e.currentTarget.style.boxShadow = "0 .7rem 1.5rem rgba(0,0,0,.3)";
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.transform = "none";
          e.currentTarget.style.boxShadow = "0 .5rem 1rem rgba(0,0,0,.2)";
        }}
      >
        {/* icône WhatsApp */}
        <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884" />
        </svg>
      </button>

      {/* Popup */}
      {isPopupOpen && (
        <>
          {/* Backdrop */}
          <div
            className="fixed inset-0 blur-2xl"
            style={{ zIndex: 1040 }}
            onClick={() => setIsPopupOpen(false)}
          />
          {/* Fenêtre chat */}
          <div
            className="fixed rounded-2xl shadow-lg bg-white overflow-hidden"
            style={{
              bottom: 90,
              right: 20,
              width: 320,
              maxWidth: "calc(100% - 2rem)",
              zIndex: 1051,
            }}
          >
            <div
              className="px-3 py-2 flex items-center justify-between"
              style={{ background: theme === "dark" ? "#000" : "#fff", color: theme === "dark" ? "#fff" : "#000" }}
            >
              <span className="font-bold">WhatsApp</span>
              <button
                onClick={() => setIsPopupOpen(false)}
                aria-label="Fermer"
                className="inline-flex h-6 w-6 items-center justify-center rounded hover:bg-black/10"
              >
                ✕
              </button>
            </div>
            <div className="p-3 bg-neutral-100">
              <div className="bg-white p-2 rounded shadow-sm max-w-[80%]">
                <p className="m-0 text-sm text-neutral-600">{message}</p>
              </div>
            </div>
            <div className="p-3 border-t">
              <button
                onClick={handleOpenWhatsApp}
                className="w-full rounded-full bg-red-500 text-white py-2 font-medium hover:opacity-90"
              >
                Lancer le chat
              </button>
            </div>
          </div>
        </>
      )}
    </>
  );

  if (!mounted) return null;
  return createPortal(content, document.body);
}
